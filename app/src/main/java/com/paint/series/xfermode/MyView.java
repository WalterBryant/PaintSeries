    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int row = 0; row < 4; row++) {
            for (int column = 0; column < 4; column++) {
                ...
                float texttextYOffset = mTextSize + (mItemVerticalOffset - mTextSize) / 2;
                canvas.drawText(text, textXOffset, texttextYOffset, mPaint);
                ...
                float reactBottom = mCircleRadius + mRectSize;
                canvas.drawRect(left, top, rectRight, reactBottom, mPaint);
                mPaint.setXfermode(null);
